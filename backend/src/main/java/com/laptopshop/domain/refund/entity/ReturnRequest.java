...
public class ReturnRequest {
    ...
    // ── NEW: Refund completion info (populated when status → REFUNDED) ────────

    /** Mã giao dịch hoàn tiền thực tế (bank transfer ref, VNPay txn id...). */
    @Column(name = "transaction_ref", length = 200)
    private String transactionRef;

    /** Phương thức hoàn tiền: BANK / CASH / MOMO / VNPAY */
    @Column(name = "refund_method", length = 50)
    private String refundMethod;

    /** Thời điểm xử lý lần cuối (approve / reject / complete). */
    ...
    @OneToMany(mappedBy = "returnRequest", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<RefundAudit> auditLogs = new ArrayList<>();
}